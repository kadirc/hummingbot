...
class CrossExchangeMarketMakingStrategy(StrategyPyBase):
    ...
    def tick(self, timestamp: float):
        ...
        if not self._conversions_ready:
            for market_pair in self._market_pairs.values():
                if not self.markettaker_to_maker_base_conversion_rate(market_pair):
                    self.logger().warning("Conversion rates are not ready. No market making trades are permitted.")
                    return
            self._conversions_ready = True
            # Markets are ready, ok to proceed.
            if LogOption.STATUS_REPORT:
                self.logger().info("Converstion rates are ready. Trading started.")
    ...
    async def check_and_hedge_orders(self, market_pair: MakerTakerMarketPair):
        ...
        if buy_fill_quantity > 0:
            ...
            hedged_order_quantity = min(
                buy_fill_quantity * self.markettaker_to_maker_base_conversion_rate(market_pair),
        ...
        if sell_fill_quantity > 0:
            ...
            if self.is_gateway_market(market_pair.taker):
                taker_price = await market_pair.taker.market.get_order_price(
                    taker_trading_pair,
                    True,
                    sell_fill_quantity * self.markettaker_to_maker_base_conversion_rate(market_pair)
                )
        ...
        if not has_active_bid:
            ...
            if bid_size > s_decimal_zero:
                ...
                if not Decimal.is_nan(bid_price):
                    ...
                    hedging_price_on_taker = effective_hedging_price / \
                    ...
                    if LogOption.CREATE_ORDER in self.logging_options:
                        self.log_with_clock(
                            ...
                            f"Current hedging price: {hedging_price_on_taker:.8f} {market_pair.taker.quote_asset} "
                            f"(Rate adjusted hedging price: {effective_hedging_price:.8f} {market_pair.maker.quote_asset})."
                        )
                ...
                if not Decimal.is_nan(ask_price):
                    ...
                    if LogOption.CREATE_ORDER in self.logging_options:
                        self.log_with_clock(
                            ...
                            f"Current hedging price: {hedging_price_on_taker:.8f} {market_pair.taker.quote_asset} "
                            f"(Rate adjusted: {effective_hedging_price:.8f} {market_pair.maker.quote_asset})."
                        )
